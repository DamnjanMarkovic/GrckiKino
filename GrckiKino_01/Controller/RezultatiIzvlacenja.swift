...
class RezultatiIzvlacenja: UIViewController {
...
//    let viewHeader = UIView()
    ...
    override func viewDidLoad() {
        ...
        configureTableView();
        vratiRealizovanaKolaPoDatumu {
            self.tableView.reloadData()
        }
    }
    
    func vratiDatum(timeAsString: String) -> Date {
    
//    let isoDateSample = "2016-04-14T10:44:00+0000"
...
//
    ...
    func vratiRealizovanaKola(completed: @escaping () -> ()) {
        let brojKola = "856666"
        let url = URL (string: "\(Constants.apiRealizovanoKolo)/\(brojKola)/")
        ...
        let _: Void = session.dataTask(with: request) { (data, response, error) in
            if error == nil {
                do {
                    self.koloPoIDju = try JSONDecoder().decode(ZavrsenoKolo.self, from: data!)
                    DispatchQueue.main.async {
                        completed()
                        self.zavrsenaKola.append(self.koloPoIDju)
